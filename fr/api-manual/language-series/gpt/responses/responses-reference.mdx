...
L’outil intégré `image_generation` est actuellement pris en charge uniquement par `gpt-6-astra` et n’est pas disponible sur les autres modèles ; pour générer des images indépendamment, vous pouvez aussi utiliser les API des modèles de la série image.
</Note>

<Note>
**Générer des images directement avec `gpt-6-astra`** : déclarez `{"type": "image_generation"}` dans `tools` ; le modèle générera des images selon les besoins de la conversation.
