    ...
    **Premier lancement : choisir le mode d'installation (Quick / Full / Blank Slate)**
    ...
    <Steps>
      ...
      <Step title="Choisir le modèle par défaut">
        À l'invite `Select model [1-N] or type name:` en bas de la liste des modèles, saisissez directement le nom du modèle à utiliser comme modèle par défaut (par ex. `claude-opus-5`) puis appuyez sur Entrée. Pour `Context length` et `Display name`, appuyez sur Entrée pour conserver les valeurs par défaut.
...
<Tabs>
  <Tab title="Assistant de configuration (recommandé)">
    ...
    <Steps>
      ...
      <Step title="Choisir le modèle par défaut">
        Enfin, saisissez le nom du modèle à utiliser comme modèle par défaut (par ex. `claude-opus-5`), puis appuyez sur Entrée pour confirmer.
