...
<Tabs>
  ...
  <Tab title="Windows">
    Exécutez la commande d'installation en une ligne dans **PowerShell** :

    ```powershell
    iex (irm https://hermes-agent.nousresearch.com/install.ps1)
    ```

    **Résultat attendu :** vous verrez les informations de téléchargement et d'installation, puis un message confirmant la réussite de l'installation.

    **En cas d'erreur :** si un message signale une restriction liée à la stratégie d'exécution, ouvrez PowerShell en tant qu'administrateur, ou exécutez d'abord `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` puis réessayez.

    Après l'installation, le premier lancement de `hermes` ouvre lui aussi l'assistant d'installation **Quick / Full** ; les étapes d'intégration d'EvoLink sont identiques à celles de l'onglet macOS / Linux (il est recommandé de choisir **Full setup** et de renseigner dans l'assistant l'adresse de l'interface EvoLink et la clé).
