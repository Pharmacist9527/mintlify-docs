...
![La page d'accueil de Pi](/images/integration-guide/pi/tui.png)

Le slogan de la page d'accueil de Pi annonce "There are many agent harnesses, but this one is yours" — Pi se positionne comme un harnais d'agent délibérément minimaliste qui s'adapte à vos flux de travail, et non l'inverse.
...
<Note>
L'objectif officiel de Pi est la **CLI de terminal** (quatre modes d'exécution : interactif / print / JSON / RPC, plus l'intégration via le SDK Node.js), et ce guide suit la CLI.
</Note>
...
<Note>
Pi nécessite **Node.js ≥ 22.19.0**. Vérifiez d'abord votre version avec `node -v` ; en dessous de cette version, `npm install -g` affiche un avertissement `EBADENGINE` et la CLI installée peut ne pas fonctionner, mettez donc d'abord à niveau Node.
</Note>
...
<Tabs>
  ...
  <Tab title="npm">
    ...
    La commande d'installation officielle inclut `--ignore-scripts` (qui ignore les scripts de cycle de vie des dépendances lors de l'installation ; l'installation normale de Pi n'en a pas besoin). Lors de la première exécution, Pi télécharge automatiquement deux outils natifs — ripgrep et fd — selon les besoins.
...
- **`authHeader: true` est requis.** Ce champ permet à Pi de joindre un en-tête de requête `Authorization: Bearer <your-key>`. Le `/v1/messages` d'EvoLink n'accepte que l'authentification Bearer, tandis que le SDK Anthropic intégré de Pi n'envoie par défaut que `x-api-key` — omettre ce champ renvoie un `401`.
- **`apiKey` a deux formulaires : choisissez-en un :**
  ...
  - **(Avancé)** L’`apiKey` de Pi prend également en charge `${EVOLINK_API_KEY}` (équivalent ; utilisez les accolades pour lever l’ambiguïté lorsque le nom de la variable est immédiatement suivi de texte littéral) et `!command` (un `!` initial exécute une commande et utilise sa sortie comme clé, par exemple pour lire depuis un gestionnaire de mots de passe : `"!op read 'op://vault/item/credential'"`). Si vous avez besoin d’un `$` ou d’un `!` littéral dans la valeur, échappez-les avec `$$` et `$!`.
...
Dans la session Pi, tapez `/model` puis appuyez sur Entrée dans la palette de commandes pour ouvrir le sélecteur de modèle :

![Saisie de la commande /model dans une session Pi](/images/integration-guide/pi/model.png)

Le sélecteur répertorie tous les modèles EvoLink configurés dans `models.json` (marqués `[evolink]`). Utilisez les touches fléchées pour mettre en surbrillance le modèle souhaité (tel que `claude-fable-5`), puis appuyez sur Entrée pour confirmer :

![Le sélecteur de modèle répertoriant les trois modèles configurés sous le fournisseur evolink](/images/integration-guide/pi/model2.png)

Une fois un modèle sélectionné, la barre d'état en bas affiche le modèle actuel, le niveau de réflexion et l'utilisation du contexte (par exemple `claude-fable-5 · medium` et `0.0%/1.0M`), confirmant que le catalogue de modèles a bien été chargé.

<Note>
L'indication en jaune "Only showing models from configured providers. Use /login to add providers." est normale — Pi ne répertorie que les modèles des fournisseurs que vous avez configurés. La connexion via le fournisseur personnalisé EvoLink ne nécessite pas `/login`.
</Note>
...
Oui. `authHeader: true` permet à Pi de joindre un en-tête de requête supplémentaire `Authorization: Bearer <your-key>` ; le `/v1/messages` d'EvoLink utilise l'authentification Bearer, tandis que le SDK Anthropic intégré de Pi n'envoie par défaut que `x-api-key`, donc l'omettre provoque un `401`.

### 3. Pourquoi ce guide suit-il la CLI du terminal ?
La forme principale officielle de Pi est la **CLI de terminal** (quatre modes d'exécution : interactif / print / JSON / RPC, plus l'intégration via le SDK Node.js). La configuration et la vérification de l'intégration d'EvoLink se font dans la CLI, qui est stable et fiable. Toutes les étapes de ce guide suivent la CLI.
