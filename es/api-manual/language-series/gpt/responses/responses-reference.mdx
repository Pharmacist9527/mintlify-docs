...
La herramienta integrada `image_generation` solo está disponible actualmente en `gpt-6-astra`, y no en los demás modelos; para generar imágenes de forma independiente, también puedes usar las API de los modelos de la serie de imagen.
</Note>

<Note>
**Genera imágenes directamente con `gpt-6-astra`**: declara `{"type": "image_generation"}` en `tools` y el modelo generará imágenes según sea necesario en la conversación.
