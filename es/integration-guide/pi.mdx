...
![La página de inicio de Pi](/images/integration-guide/pi/tui.png)

El eslogan de la página de inicio de Pi dice "There are many agent harnesses, but this one is yours" — Pi se posiciona como un arnés de agente deliberadamente minimalista que se adapta a sus flujos de trabajo, y no al revés.
...
<Note>
El enfoque oficial de Pi es la **CLI de terminal** (cuatro modos de ejecución: interactivo / impresión / JSON / RPC, más la integración mediante el SDK de Node.js), y esta guía sigue la CLI.
</Note>
...
<Note>
Pi requiere **Node.js ≥ 22.19.0**. Primero verifique su versión con `node -v`; por debajo de esta versión, `npm install -g` imprime una advertencia `EBADENGINE` y la CLI instalada puede fallar al ejecutarse, así que actualice Node primero.
</Note>
...
<Tabs>
  ...
  <Tab title="npm">
    ...
    El comando de instalación oficial incluye `--ignore-scripts` (que omite los scripts del ciclo de vida de las dependencias durante la instalación; la instalación normal de Pi no los necesita). En la primera ejecución, Pi descarga automáticamente dos herramientas nativas — ripgrep y fd — según sea necesario.
...
- **`authHeader: true` es obligatorio.** Este campo hace que Pi adjunte un encabezado de solicitud `Authorization: Bearer <your-key>`. El `/v1/messages` de EvoLink solo acepta autenticación Bearer, mientras que el SDK de Anthropic integrado en Pi solo envía `x-api-key` de forma predeterminada — omitir este campo devuelve un `401`.
- **`apiKey` tiene dos formularios: elige uno:**
  ...
  - **(Avanzado)** El `apiKey` de Pi también admite `${EVOLINK_API_KEY}` (equivalente; use llaves para eliminar la ambigüedad cuando el nombre de la variable va inmediatamente seguido de texto literal) y `!command` (un `!` inicial ejecuta un comando y usa su salida como clave, por ejemplo leyendo desde un gestor de contraseñas: `"!op read 'op://vault/item/credential'"`). Si necesita un `$` o un `!` literal en el valor, escápelos como `$$` y `$!`.
...
Dentro de la sesión de Pi, escriba `/model` y presione Enter en la paleta de comandos para abrir el selector de modelos:

![Escribiendo el comando /model dentro de una sesión de Pi](/images/integration-guide/pi/model.png)

El selector muestra todos los modelos de EvoLink configurados en `models.json` (etiquetados como `[evolink]`). Use las teclas de flecha para resaltar el modelo que desee (como `claude-fable-5`) y presione Enter para confirmar:

![El selector de modelos mostrando los tres modelos configurados bajo el proveedor evolink](/images/integration-guide/pi/model2.png)

Una vez seleccionado un modelo, la barra de estado en la parte inferior muestra el modelo actual, el nivel de razonamiento y el uso de contexto (por ejemplo, `claude-fable-5 · medium` y `0.0%/1.0M`), lo que confirma que el catálogo de modelos se cargó correctamente.

<Note>
La sugerencia amarilla "Only showing models from configured providers. Use /login to add providers." es normal: Pi solo muestra los modelos de los proveedores que usted ha configurado. Conectarse a través del proveedor personalizado de EvoLink no requiere `/login`.
</Note>
...
Sí. `authHeader: true` hace que Pi adjunte un encabezado de solicitud adicional `Authorization: Bearer <your-key>`; el `/v1/messages` de EvoLink usa autenticación Bearer, mientras que el SDK de Anthropic integrado en Pi solo envía `x-api-key` de forma predeterminada, por lo que omitirlo causa un `401`.

### 3. ¿Por qué esta guía sigue la CLI del terminal?
La forma principal oficial de Pi es la **CLI de terminal** (cuatro modos de ejecución: interactivo / impresión / JSON / RPC, más la integración mediante el SDK de Node.js). La configuración y verificación de la integración de EvoLink se realizan en la CLI, que es estable y confiable. Todos los pasos de esta guía siguen la CLI.
