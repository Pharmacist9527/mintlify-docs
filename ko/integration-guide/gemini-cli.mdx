---
title: "Gemini CLI"
description: "Gemini CLI를 EvoLink.AI에 연결하기"
---
...
Gemini CLI는 터미널에서 Gemini 모델과 상호작용하기 위한 Google 공식 커맨드라인 도구입니다.
Gemini CLI를 **EvoLink API**와 통합하면 공식 모델 제한을 넘어 **Gemini, GPT, Claude** 시리즈 모델을 포함한 EvoLink의 다양한 모델 세트에 하나의 진입점으로 접근할 수 있습니다.
...
**이미 설치된 경우:** `node -v` 및 `npm -v`를 실행하여 버전을 확인하세요. v20 이상이면 이 단계를 건너뛰세요.

**처음 설치하는 경우:**
- [Node.js 공식 웹사이트](https://nodejs.org/)에서 다운로드 및 설치 (LTS 버전 권장)
...
- [EvoLink Dashboard](https://evolink.ai/dashboard)에 로그인
...
**오류 발생 시:** `permission denied`이 나타나면 Windows에서는 "관리자 권한으로 실행"한 PowerShell을 사용하고, macOS/Linux에서는 명령어 앞에 `sudo`를 추가하세요.
...
**⚠️ 중요:** Gemini CLI 설정은 다른 도구보다 **약간 더 복잡**하며, 설치 파일을 수정해야 합니다. 프로그래밍이 처음이라면 먼저 **Claude CLI** 또는 **Codex CLI** 사용을 고려해 보세요 (설정이 더 간단합니다).
...
**파일 1:** `{install_dir}\@google\gemini-cli\node_modules\@google\genai\dist\node\index.mjs`
...
**파일 2:** `{install_dir}\@google\gemini-cli\node_modules\@google\genai\dist\node\index.cjs`
...
**참고:** `your-working-directory`를 실제 경로로 바꾸세요
...
- **401 오류**: `GEMINI_API_KEY`가 설정되지 않았거나 유효하지 않음
- **403 오류**: 권한 부족 또는 만료된 키
- API 엔드포인트가 `https://direct.evolink.ai/`인지 확인하세요
