...
![Pi のホームページ](/images/integration-guide/pi/tui.png)

Pi のホームページには "There are many agent harnesses, but this one is yours" というタグラインが掲げられています。Pi は、ユーザーがツールに合わせるのではなく、ツールがユーザーのワークフローに適応する、意図的にミニマルなエージェント ハーネスとして自らを位置づけています。
...
<Note>
Pi の公式の焦点は **ターミナル CLI** (4 つの実行モード: インタラクティブ / プリント / JSON / RPC。さらに Node.js SDK 経由での組み込みも可能) にあり、本ガイドも CLI に沿って進めます。
</Note>
...
<Note>
Pi には **Node.js ≥ 22.19.0** が必要です。まず `node -v` でバージョンを確認してください。これより低いバージョンでは、`npm install -g` の実行時に `EBADENGINE` 警告が表示され、インストールされた CLI が正常に動作しない可能性があるため、先に Node.js をアップグレードしてください。
</Note>
...
<Tabs>
  ...
  <Tab title="npm">
    ...
    公式のインストール コマンドには、`--ignore-scripts` が含まれています (インストール中に依存関係のライフサイクル スクリプトをスキップします。Pi の通常のインストールではそれらは必要ありません)。初回実行時に、Pi は必要に応じて ripgrep と fd の 2 つのネイティブ ツールを自動的にダウンロードします。
...
- **`authHeader: true` は必須です。** このフィールドを指定すると、Pi は `Authorization: Bearer <your-key>` リクエスト ヘッダーを付与します。EvoLink の `/v1/messages` は Bearer 認証のみを受け付ける一方、Pi 内蔵の Anthropic SDK はデフォルトで `x-api-key` しか送信しないため、このフィールドを省略すると `401` が返されます。
- **`apiKey` には 2 つの形式があります - 1 つを選択してください:**
  ...
  - **（上級）** Pi の `apiKey` は `${EVOLINK_API_KEY}`（同等の書き方。変数名の直後にリテラル文字列が続く場合は波かっこで区切ります）と `!command`（先頭が `!` の場合はコマンドを実行し、その出力をキーとして使用します。例：パスワードマネージャーから読み取る `"!op read 'op://vault/item/credential'"`）にも対応しています。値にリテラルの `$` や `!` が必要な場合は、`$$` と `$!` でエスケープしてください。
...
Pi セッション内で、コマンド パレットに `/model` と入力して Enter キーを押すと、モデル セレクターが開きます。

![Pi セッション内で /model コマンドを入力しているところ](/images/integration-guide/pi/model.png)

セレクターには、`models.json` で構成したすべての EvoLink モデルが (`[evolink]` タグ付きで) 一覧表示されます。矢印キーで使用したいモデル (`claude-fable-5` など) をハイライトし、Enter キーを押して確定します。

![evolink プロバイダー配下に構成された 3 つのモデルを一覧表示するモデル セレクター](/images/integration-guide/pi/model2.png)

モデルを選択すると、画面下部のステータス バーに現在のモデル、思考レベル、コンテキスト使用量 (例: `claude-fable-5 · medium` と `0.0%/1.0M`) が表示され、モデル カタログが正常に読み込まれたことを確認できます。

<Note>
黄色のヒント "Only showing models from configured providers. Use /login to add providers." は想定どおりの表示です。Pi は構成済みのプロバイダーのモデルのみを一覧表示します。EvoLink カスタム プロバイダー経由で接続する場合、`/login` は必要ありません。
</Note>
...
はい。`authHeader: true` を指定すると、Pi は追加の `Authorization: Bearer <your-key>` リクエスト ヘッダーを付与します。EvoLink の `/v1/messages` は Bearer 認証を使用する一方、Pi 内蔵の Anthropic SDK はデフォルトで `x-api-key` しか送信しないため、これを省略すると `401` が発生します。

### 3. このガイドが端末 CLI に従っているのはなぜですか?
Pi の公式の主な形式は **ターミナル CLI** (4 つの実行モード: インタラクティブ / プリント / JSON / RPC。さらに Node.js SDK 経由での組み込みも可能) です。EvoLink 統合の構成と検証はすべて CLI 上で行うため、本ガイドの手順も CLI に沿っています。
