...
**重要：** Gemini CLI の設定は他のツールより**やや複雑**で、インストールファイルの修正が必要です。プログラミング初心者の方は、まず **Claude CLI** や **Codex CLI**（設定がより簡単）の使用を検討してください。
...
- API エンドポイントが `https://direct.evolink.ai/` であることを確認してください
