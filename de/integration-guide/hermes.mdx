...
<Tabs>
  ...
  <Tab title="Windows">
    Führen Sie den Installationsbefehl in **PowerShell** aus:

    ```powershell
    iex (irm https://hermes-agent.nousresearch.com/install.ps1)
    ```

    **Erwartetes Ergebnis:** Sie sehen Download- und Installationsinformationen und am Ende einen Hinweis auf die erfolgreiche Installation.

    **Falls ein Fehler auftritt:** Wenn eine Einschränkung der Ausführungsrichtlinie gemeldet wird, öffnen Sie PowerShell als Administrator oder führen Sie zuerst `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` aus und versuchen Sie es erneut.

    Beim ersten Ausführen von `hermes` nach der Installation startet ebenfalls der **Quick / Full** Installationsassistent. Die Schritte zur Anbindung von EvoLink sind identisch mit dem Tab macOS / Linux (es wird empfohlen, **Full setup** zu wählen und die EvoLink-Schnittstellenadresse und den Schlüssel im Assistenten einzugeben).
