...
Das integrierte Werkzeug `image_generation` wird derzeit nur von `gpt-6-astra` unterstützt und ist auf den anderen Modellen nicht verfügbar; für die eigenständige Bildgenerierung können Sie auch die APIs der Bildmodellreihe verwenden.
</Note>

<Note>
**Bilder direkt mit `gpt-6-astra` generieren**: Deklarieren Sie `{"type": "image_generation"}` in `tools`. Das Modell generiert dann bei Bedarf Bilder innerhalb der Konversation.
