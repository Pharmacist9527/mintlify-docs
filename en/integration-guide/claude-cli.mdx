...
### Open Claude Code config directory

<Tabs>
  <Tab title="Windows">
    Press `Win + R`, paste the following, then press Enter to open the Claude Code config directory:
    ```
    %userprofile%\.claude
    ```

    ![Open Configuration Directory](/images/integration-guide/image-copy-2.png)

    If `settings.json` doesn't exist, create it first.
  </Tab>
  <Tab title="macOS">
    ...
    If the folder (or `settings.json`) doesn’t exist, create it first.
  </Tab>
  <Tab title="Linux">
    The config directory is typically:
    ```
    ~/.claude
    ```
...
![Configuration Directory Structure](/images/integration-guide/image-copy-3.png)

`settings.json` is Claude Code's main configuration file for your proxy base URL, API key, and optional hooks/plugins.
