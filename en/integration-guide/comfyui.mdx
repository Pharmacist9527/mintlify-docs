...
|---|---|
| `prompt` | Description / edit instruction; `gpt-image-2` allows up to 32,000 characters and no more than 60,000 bytes when encoded in UTF-8 |
...
<Note>
  For `gpt-image-2`, the `prompt` must meet both the character and byte limits. Different characters use different numbers of UTF-8 bytes, so use the actual encoded byte count. If either limit is exceeded, shorten the prompt before resubmitting.
</Note>
