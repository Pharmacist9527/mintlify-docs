...
<Tabs>
  <Tab title="Method 1: GUI Operation">
    ...
    **Configuration options:**
    - `ANTHROPIC_AUTH_TOKEN`: Your EvoLink API Key
    - `ANTHROPIC_BASE_URL`: `https://api.evolink.ai` (EvoLink API endpoint)
...
"@ | Out-File -FilePath "$env:USERPROFILE\.claude\settings.json" -Encoding utf8
        ```
    ![Claude Code CLI one-command configuration](/images/integration-guide/claude-one-command-config.png)
      </Tab>
      <Tab title="macOS / Linux">
        ```bash
