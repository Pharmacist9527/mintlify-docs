...
### 1. Open Gemini CLI configuration directory (.gemini)

<Tabs>
  <Tab title="Windows">
    Press `Win + R`, enter the following and press Enter to open the Gemini CLI configuration directory:
    ```
    %userprofile%\.gemini
    ```
  </Tab>
  <Tab title="macOS">
    In Finder, press `Command + Shift + G`, enter the following path and press Enter:
    ```
    ~/.gemini
    ```
  </Tab>
...
If there is no `.env` file in the directory, create a new `.env` file and add the following content.

`.env`: Gemini CLI configuration file for setting custom endpoint, API Key, and model.

Example:

```env
GOOGLE_GEMINI_BASE_URL="https://api.evolink.ai/"
GEMINI_API_KEY="your-evolink-api-key"
GEMINI_MODEL="gemini-2.5-pro"
```

Configuration options:
- `GOOGLE_GEMINI_BASE_URL`: Custom endpoint (EvoLink)
- `GEMINI_API_KEY`: Your EvoLink API Key
- `GEMINI_MODEL`: Default model (can also switch using `/model` in interactive mode)

<Tip>
Gemini CLI automatically loads `.gemini/.env` (and searches upward for `.env` in project directories). After modifying `.env`, exit and restart `gemini` for changes to take effect.
</Tip>
...
### 8. Why don't my `.env` configuration changes take effect?
- Exit and restart Gemini CLI
- Confirm `.env` file is located in `~/.gemini/` directory
- Confirm `GOOGLE_GEMINI_BASE_URL` and `GEMINI_API_KEY` are set correctly
- Check `.env` file format is correct (one configuration per line, format: `KEY="value"`)
