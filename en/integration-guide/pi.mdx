...
description: "Connect Pi Coding Agent to EvoLink.AI"
---

## Overview

![The Pi homepage](/images/integration-guide/pi/tui.png)
...
<Tabs>
  <Tab title="curl script">
    ...
    ![Installing Pi with the curl script](/images/integration-guide/pi/curl-install.png)
  </Tab>
  <Tab title="npm">
    ```bash
    ...
    ![Installing Pi with npm](/images/integration-guide/pi/npm-install.png)
...
- **`authHeader: true` is required.** This field makes Pi attach an `Authorization: Bearer <your-key>` request header. EvoLink's `/v1/messages` only accepts Bearer authentication, while Pi's built-in Anthropic SDK only sends `x-api-key` by default — omitting this field returns a `401`.
...
![Typing the /model command inside a Pi session](/images/integration-guide/pi/model.png)

The selector lists every EvoLink model configured in `models.json` (tagged `[evolink]`). Use the arrow keys to highlight the model you want (such as `claude-fable-5`) and press Enter to confirm:

![The model selector listing the three models configured under the evolink provider](/images/integration-guide/pi/model2.png)
...
<Note>
The yellow hint "Only showing models from configured providers. Use /login to add providers." is expected — Pi only lists models from providers you have configured. Connecting through the EvoLink custom provider does not require `/login`.
</Note>
...
![Pi responding normally to “who are you”](/images/integration-guide/pi/whoareyou2.png)
...
Yes. `authHeader: true` makes Pi attach an additional `Authorization: Bearer <your-key>` request header; EvoLink's `/v1/messages` uses Bearer authentication, while Pi's built-in Anthropic SDK only sends `x-api-key` by default, so omitting it causes a `401`.

### 3. Why does this guide follow the terminal CLI?
Pi's official primary form is the **terminal CLI** (four run modes: interactive / print / JSON / RPC, plus embedding via the Node.js SDK). Configuring and verifying the EvoLink integration is all done in the CLI, which is stable and reliable. All steps in this guide follow the CLI.
