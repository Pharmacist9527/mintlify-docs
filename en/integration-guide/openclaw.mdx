...
<Note>
`gpt-5-mini` is an example model ID. Please use the actual models available in your EvoLink account.
</Note>

<Warning>
For Gemini scenarios, `google.baseUrl` must include `/v1beta`, i.e., `https://direct.evolink.ai/v1beta`. Without this suffix, you may encounter `Forbidden (403)` errors.
</Warning>
...
- Switch to EvoLink model: `evolink/gpt-5.2`
- Switch to Gemini: `google/gemini-3.1-pro-preview`
- Switch to Claude: `anthropic/claude-opus-4-6`
...
```bash
# View configured Evolink models
openclaw models list --provider evolink --plain

# Switch default model (example: gpt-5-mini)
...
<Note>
If `models list --provider evolink` doesn't show your expected models, check if both `openclaw.json` and `models.json` have the corresponding provider configured.
</Note>
...
| Gemini returns `Forbidden (403)` | Check if `models.providers.google.baseUrl` is `https://direct.evolink.ai/v1beta` (must include `/v1beta`) |
