...
<Tabs>
  ...
  <Tab title="Windows">
    Run the one-line install command in **PowerShell**:

    ```powershell
    iex (irm https://hermes-agent.nousresearch.com/install.ps1)
    ```

    **Expected result:** You'll see download and installation information, ending with an installation success message.

    **If it errors:** If you get an execution policy restriction, open PowerShell as an administrator, or run `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` first and then retry.

    Running `hermes` for the first time after installation also launches the **Quick / Full** install wizard. The steps for connecting EvoLink are the same as in the macOS / Linux tab (we recommend choosing **Full setup** and entering the EvoLink endpoint URL and key in the wizard).
