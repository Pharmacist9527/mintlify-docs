    ...
    **First run: choose an installation method (Quick / Full / Blank Slate)**
    ...
    <Steps>
      ...
      <Step title="Choose a default model">
        At the `Select model [1-N] or type name:` prompt at the bottom of the model list, type the name of the model you want as the default (such as `claude-opus-5-5`) and press Enter. You can press Enter to accept the defaults for `Context length` and `Display name`.

        <Frame>
        ![Typing the default model name in the model list](/images/integration-guide/hermes/windows-hermes-agent-custom-endpoint2.png)
        </Frame>
      </Step>
    </Steps>
...
<Tabs>
  <Tab title="Setup Wizard (Recommended)">
    ...
    <Steps>
      ...
      <Step title="Choose a default model">
        Finally, enter the name of the model you want as the default (such as `claude-opus-5-5`) and press Enter to confirm.
...
| `claude-fable-5-1` | Latest version |
| `claude-opus-5-5` | Latest version |
