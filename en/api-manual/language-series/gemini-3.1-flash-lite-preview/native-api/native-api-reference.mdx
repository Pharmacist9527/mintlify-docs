---
title: "Gemini 3.1 Flash Lite - Native API - Full Reference"
openapi: '/en/api-manual/language-series/gemini-3.1-flash-lite-preview/native-api/native-api-reference.json POST /v1beta/models/gemini-3.1-flash-lite-preview:generateContent'
---

<Tip>
**Streaming**: Replace `generateContent` with `streamGenerateContent` in the URL. The request body parameters are identical. Responses will be returned in streaming chunks. See the "Streaming Response" section below for the response format.
</Tip>
