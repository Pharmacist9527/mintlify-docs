...
Midjourney V8.2 is an update centred on aesthetics and image quality: results are more creative, bold and refined, and low-quality outliers are much rarer. On this platform V8.2 uses the **same seven model IDs, the same endpoints and the same prices** as V8.1, so migrating is a one-line change: replace `mj-v8.1` with `mj-v8.2` in the `model` field. Prompts do not need to be rewritten.
...
| Speed modes | `draft` / `fast` | `draft` / `fast` (turbo is not available on either) |
...
| Prices and multipliers | 5.4 / 8.1 Credits, `hd` x1.5 | Identical |
| Derived tasks (variation / remix / edit) | Rendered by the V7 engine upstream (per the upstream channel's documentation) | Same |
...
| Personalization | `--p [code]` | Code | — | — | Apply a personalization profile or moodboard code, passed through as-is |
...
Sets the image detail level. Range 1 - 4, default 1; `4` is the high-quality mode. The flag is passed through to the upstream as-is and **does not change the price**.
...
Lists elements that should not appear in the image. Separate multiple items with commas. It is equivalent to giving those elements a weight of -0.5. Writing "no fruit" or "without fruit" in the description does not work — Midjourney treats those words as content — so use the flag instead.
...
Applies a Midjourney personalization profile or moodboard code. The flag is passed through unchanged. Profiles and moodboards are created on midjourney.com; the API cannot create or list them, so only use codes you already have and validate the effect on a small batch first. `--p` cannot be combined with `--weird`.
...
<Note>
Unlike V7, V8.2 `draft` mode is **not** half price — it uses the same multiplier as `fast`. Draft returns 24 small sketches in one run; pick the ones you like and re-run them at full quality. Draft cannot be combined with `hd` quality, `--tile`, image prompts or `--sref`.
