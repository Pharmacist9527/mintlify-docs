...
2. **Poll** — `GET /v1/tasks/{task_id}` ([Query Task Status](/en/api-manual/task-management/get-task-detail)). The upstream reports an average of about 40 seconds for generation and edits; poll every 3–5 seconds with a client-side cap of about 20 minutes, and do not resubmit the same prompt while a task is still `processing`. The upstream limits concurrency per organization account; when the limit is exceeded, this platform backs off and resubmits automatically, so on your side the task only takes longer rather than failing.
...
| `failed` | Upstream error, parameter rejection, moderation or fetch failure; the message is in `error` and reserved credits are refunded — except when moderation blocked every image, see below | Yes |

Failed tasks are refunded in full, including requests rejected at creation, upstream parameter rejections (`--oref`, out-of-range `--q`, too-long description) and image fetch failures. The one exception: if upstream content moderation blocks **all** images of a task, the task ends as `failed` and the credits are **not** refunded, because the upstream has already billed it. If only some images are blocked, the task is `completed` and the blocked slots hold an empty URL. Error codes and retry guidance are listed on the [Error Codes](/en/api-manual/task-management/error-codes) page.
...
<Note>
Midjourney has a built-in content moderation system. Each image is moderated individually: if some of the generated images are filtered, the task is still `completed`, their slots are returned empty and the remaining images are delivered normally; if **all** images are filtered, the task ends as `failed`. In both cases the credits consumed for that request are not refunded, because the upstream has already billed it. Please ensure your prompts comply with content guidelines.
</Note>
