...
| Unsupported parameters | Silently stripped | Passed through; the upstream rejects them with an explicit error and the reserved credits are refunded |
...
Applies a Midjourney personalization profile or moodboard code. The parameter is passed through unchanged. Profiles and moodboards are created on midjourney.com; the API cannot create or list them, so only use codes you already have and validate the effect on a small batch first. `--p` cannot be combined with `--weird`.
