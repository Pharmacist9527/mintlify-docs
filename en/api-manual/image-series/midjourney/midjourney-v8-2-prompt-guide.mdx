...
icon: "book-open"
description: "All available parameters for the Midjourney V8.2 model in prompts, including value ranges, defaults, dependencies, and conflicts"
---
...
<Note>
Three settings are **not** written into the prompt — they are controlled through the API instead (writing them in the prompt has no effect, the flags are stripped):

- **Speed** (`draft` / `fast` / `turbo`) → `model_params.speed`
- **Output quality** (`standard` / `hd`) → top-level `quality` parameter
- **Detail level** (`1` – `4`) → `model_params.detail`

`--v` / `--version` is locked to V8.2 and `--niji` is not supported. See [Speed Modes](#speed-modes), [Output Quality](#output-quality) and [Detail Level](#detail-level) below.
</Note>
...
<Note>
Unlike V7, V8.2 `draft` mode is **not** half price — it uses the same multiplier as `fast`. Draft returns 24 small sketches in one run; pick the ones you like and re-run them at full quality. Draft cannot be combined with `hd` quality.
</Note>
...
The detail level is controlled via the API's `model_params.detail` field, **not** a prompt flag. Do not write `--q` / `--quality` in the prompt.

| Value | Description | Cost |
|-------|-------------|------|
| `1` | Default detail | 1x |
| `2` – `4` | Progressively richer detail, longer render time | 1x (no extra cost) |

- Maps to Midjourney's native `--quality N`; V8.2 accepts 1 / 2 / 3 / 4 (V8.1 only accepted 1 / 4).
- Image generation endpoint only; derived tasks inherit the source image.
- Detail level does **not** change the price.
...
| `--q` / `--quality` | Stripped — use `model_params.detail` |
