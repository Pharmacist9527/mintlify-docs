...
<Tabs>
  <Tab title="Windows">
    键盘按下 `Win + R` 键，输入以下内容后回车，打开 Claude Code 配置目录：
    ```
    %userprofile%\.claude
    ```

    ![打开配置目录](/images/integration-guide/image-copy-2.png)

    如果目录中没有 `settings.json`，需要你手动创建后打开编辑。
  </Tab>
  <Tab title="macOS">
    ...
    若目录不存在或没有 `settings.json`，需要你手动进行创建。
  </Tab>
  <Tab title="Linux">
    配置目录通常为：
    ```
...
![配置目录结构](/images/integration-guide/image-copy-3.png)

`settings.json`：Claude 主要的配置文件，主要用来配置中转站地址以及 ApiKey，以及一些 hooks、plugins 等。

### 编辑 settings.json

`settings.json` 是 Claude CLI 的主要配置文件，用于配置 API 地址、API Key，以及 hooks、plugins 等。

在配置目录中创建或编辑 `settings.json` 文件，添加以下内容：
