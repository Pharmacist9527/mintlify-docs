...
![Claude CLI 版本](/images/integration-guide/image-copy.png)

## 第二步：配置 EvoLink API

Claude CLI 通过 `settings.json` 配置文件进行配置。
...
<Tabs>
  <Tab title="方法一：图形界面操作">
    ...
    ```
    ![Claude CLI 配置环境变量](/images/integration-guide/claude-config-env.png)
    **⚠️ 重要提示：**
    ...
    **配置项说明：**
    ...
    - `ANTHROPIC_BASE_URL`：EvoLink API 接口地址
...
"@ | Out-File -FilePath "$env:USERPROFILE\.claude\settings.json" -Encoding utf8
    ...
    ![Claude CLI 一键配置](/images/integration-guide/claude-one-command-config.png)
      </Tab>
      <Tab title="macOS / Linux">
        ```bash
...
![Claude CLI 界面展示](/images/integration-guide/claude-interface.png)
