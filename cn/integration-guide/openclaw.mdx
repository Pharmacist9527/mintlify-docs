---
title: "OpenClaw 手动安装"
description: "手动安装和配置 OpenClaw Gateway"
---
...
<Note>
`gpt-5-mini` 为示例模型 ID。请以你的 EvoLink 账户实际开通模型为准。
</Note>

<Warning>
Gemini 场景下 `google.baseUrl` 必须带 `/v1beta`，即 `https://direct.evolink.ai/v1beta`。若缺少该后缀,常见报错为 `Forbidden (403)`。
</Warning>

### 3. 配置默认模型（支持快速切换）

在 `agents.defaults` 中设置默认模型:
...
- 切换到 EvoLink 模型: `evolink/gpt-5.2`
- 切换到 Gemini: `google/gemini-3.1-pro-preview`
- 切换到 Claude: `anthropic/claude-opus-4-6`
...
<Note>
如果 `models list --provider evolink` 看不到你预期的模型，请先检查 `openclaw.json` 与 `models.json` 两处是否都已配置对应 provider。
</Note>
...
| Gemini 返回 `Forbidden (403)` | 检查 `models.providers.google.baseUrl` 是否为 `https://direct.evolink.ai/v1beta`（必须带 `/v1beta`） |
