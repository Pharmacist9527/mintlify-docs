...
### 1. 打开 Gemini CLI 配置目录（.gemini）

<Tabs>
  <Tab title="Windows">
    键盘按下 `Win + R`，输入以下内容后回车，打开 Gemini CLI 配置目录：
    ```
    %userprofile%\.gemini
    ```
  </Tab>
...
若目录中没有 `.env` 文件，请新建一个 `.env`，写入以下内容。
...
GEMINI_API_KEY="你的EvoLink API Key"
GEMINI_MODEL="gemini-2.5-pro"
```

配置项说明：
- `GOOGLE_GEMINI_BASE_URL`：自定义端点（EvoLink）
- `GEMINI_API_KEY`：你的 EvoLink API Key
- `GEMINI_MODEL`：默认模型（也可在交互模式用 `/model` 切换）

<Tip>
Gemini CLI 会自动加载 `.gemini/.env`（以及项目目录向上查找的 `.env`）。修改 `.env` 后，退出并重新启动 `gemini` 使其生效。
</Tip>
...
### 8. 修改了 `.env` 配置文件后，为什么没有生效？
- 需要退出并重新启动 Gemini CLI
- 确认 `.env` 文件位于 `~/.gemini/` 目录下
- 确认 `GOOGLE_GEMINI_BASE_URL` 和 `GEMINI_API_KEY` 已正确设置
- 检查 `.env` 文件格式是否正确（每行一个配置项，格式为 `KEY="value"`）
