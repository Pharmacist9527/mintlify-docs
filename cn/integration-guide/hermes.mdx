...
- **模型上下文要求**：Hermes Agent 要求所使用的模型**至少具备 64K（64,000）tokens 的上下文长度**，否则会在启动时被拒绝。这是因为系统提示词和工具定义本身就会占用较大上下文。请选用满足该要求的模型（EvoLink 的 Claude / Gemini 系列均满足）。
...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    **如果出错：** 如提示执行策略限制，请以管理员身份打开 PowerShell，或先执行 `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` 后重试。

    安装后首次运行 `hermes` 同样会进入 **Quick / Full** 安装向导，接入 EvoLink 的步骤与 macOS / Linux 标签页一致（推荐选择 **Full setup**，在向导中填入 EvoLink 接口地址与 Key）。
...
Hermes Agent 要求模型至少具备 64K tokens 上下文，否则会在启动时拒绝。请换用满足该要求的模型（如 EvoLink 的 Claude 系列）。
